...
ap.add_argument("-e1", "--epochs", type=int, default=100,
...
ap.add_argument("-e6", "--local_mem_dim", type=int,
                default=500, help="# of local_mem_dim")
ap.add_argument("-e7", "--filter_size0", type=int,
                default=1, help="# of filter_size0")
ap.add_argument("-e8", "--filter_size1", type=int,
                default=32, help="# of filter_size1")
ap.add_argument("-e9", "--filter_size2", type=int,
                default=64, help="# of filter_size2")
ap.add_argument("-e10", "--filter_size3", type=int,
                default=128, help="# of filter_size3")
ap.add_argument("-e11", "--lambda1", type=float,
                default=1.0, help="# of lambda1")
...
class Memory_local(Layer):
    ...
    def call(self, inputs):
        x1_size = inputs.shape[1]  # x1_size = K.int_shape(inputs)[1]
        x2_size = inputs.shape[2]  # x2_size = K.int_shape(inputs)[2]
        channel = inputs.shape[3]  # channel = K.int_shape(inputs)[3]

        inputs = tf.reshape(inputs, [-1, channel])

        distance = tf.linalg.matmul(inputs, tf.transpose(
            self.weight))  # Fea x Mem^T, (TxC) x (CxM) = TxM
        att_weight = softmax(distance, axis=1)

        # AttWeight x Mem^T^T = AW x Mem, (TxM) x (MxC) = TxC
        output = tf.linalg.matmul(att_weight, self.weight)
        output = tf.reshape(output, [-1, x1_size, x2_size, channel])
        att = tf.reshape(att_weight, [-1,  x1_size * x2_size * self.fea_dim])
        att = tf.reduce_mean(-att * tf.math.log(att), axis=-1, keepdims=True)
        return [output, att]
...
def conlstm_auto():
    ...
    model_global = Model(inputs=inp, outputs=[
                         memory_output, att_weight], name='global_memory')
    ...
    ############################# local memory########################
    memory_output1, att_weight1 = Memory_local(
        mem_dim=filter_size2, fea_dim=local_mem_dim, name='memory_local_1')(x1_)
    memory_output2, att_weight2 = Memory_local(
        mem_dim=filter_size2, fea_dim=local_mem_dim, name='memory_local_2')(x2_)
    memory_output3, att_weight3 = Memory_local(
        mem_dim=filter_size2, fea_dim=local_mem_dim, name='memory_local_3')(x3_)
    memory_output4, att_weight4 = Memory_local(
        mem_dim=filter_size2, fea_dim=local_mem_dim, name='memory_local_4')(x4_)
    memory_output5, att_weight5 = Memory_local(
        mem_dim=filter_size2, fea_dim=local_mem_dim, name='memory_local_5')(x5_)
    memory_output6, att_weight6 = Memory_local(
        mem_dim=filter_size2, fea_dim=local_mem_dim, name='memory_local_6')(x6_)
    memory_output7, att_weight7 = Memory_local(
        mem_dim=filter_size2, fea_dim=local_mem_dim, name='memory_local_7')(x7_)
    ...
    memory_output_raw = Concatenate(
        axis=-1, name='concat_local1_uni')([x1_, final_1])
    memory_output_no = Concatenate(
        axis=-1, name='concat_local2_uni')([x2_, final_2])
    memory_output_ne = Concatenate(
        axis=-1, name='concat_local3_uni')([x3_, final_3])
    memory_output_op = Concatenate(
        axis=-1, name='concat_local4_uni')([x4_, final_4])
    memory_output_pe = Concatenate(
        axis=-1, name='concat_local5_uni')([x5_, final_5])
    memory_output_sc = Concatenate(
        axis=-1, name='concat_local6_uni')([x6_, final_6])
    memory_output_ti = Concatenate(
        axis=-1, name='concat_local7_uni')([x7_, final_7])

    ############################# decoder########################

    xx1_l = Conv2DTranspose(filter_size3, (4, 4), padding='same',
                            activation='relu', name='transpose1_1')(memory_output_raw)
    xx1_l = Conv2DTranspose(filter_size2, (4, 4), padding='same', strides=(
        2, 2), activation='relu', name='transpose1_2')(xx1_l)
    xx1_l = Conv2DTranspose(filter_size1, (4, 4), padding='same', strides=(
    ...
    xx2_l = Conv2DTranspose(filter_size2, (4, 4), padding='same', strides=(
        2, 2), activation='relu', name='transpose2_2')(xx2_l)
    xx2_l = Conv2DTranspose(filter_size1, (4, 4), padding='same', strides=(
    ...
    xx3_l = Conv2DTranspose(filter_size2, (4, 4), padding='same', strides=(
        2, 2), activation='relu', name='transpose3_2')(xx3_l)
    xx3_l = Conv2DTranspose(filter_size1, (4, 4), padding='same', strides=(
    ...
    xx4_l = Conv2DTranspose(filter_size2, (4, 4), padding='same', strides=(
        2, 2), activation='relu', name='transpose4_2')(xx4_l)
    xx4_l = Conv2DTranspose(filter_size1, (4, 4), padding='same', strides=(
    ...
    xx5_l = Conv2DTranspose(filter_size2, (4, 4), padding='same', strides=(
        2, 2), activation='relu', name='transpose5_2')(xx5_l)
    xx5_l = Conv2DTranspose(filter_size1, (4, 4), padding='same', strides=(
    ...
    xx6_l = Conv2DTranspose(filter_size2, (4, 4), padding='same', strides=(
        2, 2), activation='relu', name='transpose6_2')(xx6_l)
    xx6_l = Conv2DTranspose(filter_size1, (4, 4), padding='same', strides=(
    ...
    xx7_l = Conv2DTranspose(filter_size2, (4, 4), padding='same', strides=(
        2, 2), activation='relu', name='transpose7_2')(xx7_l)
    xx7_l = Conv2DTranspose(filter_size1, (4, 4), padding='same', strides=(
...
if __name__ == '__main__':
    ...
    filepath = model_path+'model_best.weights.h5'
    ...
    y_classes = to_categorical(label)
    n = X_train.shape[0]
    y_1 = y_classes[:n]
    y_2 = y_classes[n:n*2]
    ...
    [predict_label8, predict_label9, predict_label1, predict_label2, predict_label3, predict_label4, predict_label5,
        predict_label6, predict_label7] = model.predict([X_train, initial_c], batch_size=batch, verbose=1)
    ...
    class_loss = predict_label1[:, 0] + predict_label2[:, 1] + predict_label3[:, 2] + predict_label4[:,
                                                                                                     3] + predict_label5[:,
                                                                                                                         4] + predict_label6[:,
                                                                                                                                             5] + predict_label7[:,
                                                                                                                                                                 6]
    np.savetxt(model_path + 'train_normal_loss_sparse.csv',
               predict_label9, delimiter=',')
    np.savetxt(model_path + 'train_normal_loss_class.csv',
               class_loss, delimiter=',')
    ...
    [predict_label8, predict_label9, predict_label1, predict_label2, predict_label3, predict_label4, predict_label5,
        predict_label6, predict_label7] = model.predict([X_test, initial_c_test], batch_size=batch, verbose=1)
    ...
    class_loss = predict_label1[:, 0] + predict_label2[:, 1] + predict_label3[:, 2] + predict_label4[:,
                                                                                                     3] + predict_label5[:,
                                                                                                                         4] + predict_label6[:,
                                                                                                                                             5] + predict_label7[:,
                                                                                                                                                                 6]
    np.savetxt(model_path + 'normal_loss_sparse.csv',
               predict_label9, delimiter=',')
    ...
    np.savetxt(model_path + 'abnormal_loss_sparse.csv',
               predict_label9, delimiter=',')
    np.savetxt(model_path + 'abnormal_loss_class.csv',
               class_loss, delimiter=',')
    np.savetxt(model_path + 'abnormal_loss_sum_mse.csv',
               predict_label8, delimiter=',')
